...
var dictDefs ={"guestProperty":"10042", "businessTypeId": "10445"};
...
function setInitData(){
    ...
    if(supplier.idCardUrl) {
    	nui.get("idCardUrl").setValue(supplier.idCardUrl);

        $("#idNoImg").attr("src",supplier.idCardUrl);
    }
    if(supplier.licenseUrl) {
    	nui.get("licenseUrl").setValue(supplier.licenseUrl);
...
/*    if(settleId =='020501'){
    ...
    if(settleId =='020501'){//&& settleId !='020501'
	    ...
	    //个人且不是现结
		if(guestProperty=='013902'&& settleId !='020501'){//&& settleId !='020501'
	    	...
	    	for(var key in requiredField3)
	    	{
	    		if(!data[key] || data[key].trim().length==0)
	    		{
	    			parent.showMsg(requiredField3[key]+"不能为空","W");
	    			return;
	    		}
	    	}
	    }
...
function onSettTypeIdChange(e){
	...
	if(settleId == '020501') {
		...
		if(settleId == '020501' && guestProperty=='013902'){
			$('#idNo').css("display","none");
			$('#lince').css("display","none");
			$('#otherPicture').css("display","none");
		}
