...
function initData(main, storeId, brandIdList, qualityIdList, compBrandIdList){
	nstoreId = storeId;
	enterMain = main;
	brandList = brandIdList;
	qualityList = qualityIdList;
	compBrandList = compBrandIdList
...
function sure() {
	...
	if (data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			var enterPrice = data[i].单价;
			var noTaxPrice = data[i].不含税单价;
			var taxPrice = data[i].含税单价;
			var taxCostPrice = data[i].人进价单价;
			var noTaxCostPrice = data[i].库存商品单价;
			var expEnterPrice = data[i].采购成本单价;
			var setCostPrice = data[i].定价成本单价;
			var sellPrice = data[i].销售价;
			...
			if(enterPrice == null || enterPrice == "" || enterPrice<0) {
				showMsg("第"+(i+1)+"行记录的单价信息有误!","W");
				return;
			}
			if(noTaxPrice == null || noTaxPrice == "" || noTaxPrice<0) {
				showMsg("第"+(i+1)+"行记录的不含税单价信息有误!","W");
				return;
			}
			if(taxPrice == null || taxPrice == "" || taxPrice<0) {
				showMsg("第"+(i+1)+"行记录的含税单价信息有误!","W");
				return;
			}
			if(taxCostPrice == null || taxCostPrice == "" || taxCostPrice<0) {
				showMsg("第"+(i+1)+"行记录的人进价单价信息有误!","W");
				return;
			}
			if(noTaxCostPrice == null || noTaxCostPrice == "" || noTaxCostPrice<0) {
				showMsg("第"+(i+1)+"行记录的库存商品单价信息有误!","W");
				return;
			}
			if(expEnterPrice == null || expEnterPrice == "" || expEnterPrice<0) {
				showMsg("第"+(i+1)+"行记录的采购成本单价信息有误!","W");
				return;
			}
			if(setCostPrice == null || setCostPrice == "" || setCostPrice<0) {
				showMsg("第"+(i+1)+"行记录的定价成本单价信息有误!","W");
				return;
			}
			if(sellPrice == null || sellPrice == "" || sellPrice<0) {
				showMsg("第"+(i+1)+"行记录的销售价信息有误!","W");
				return;
			}
			...
			var newRow = {
				...
				noTaxPrice : (data[i].不含税单价||"").replace(/\s+/g, ""),
				taxPrice : (data[i].含税单价||"").replace(/\s+/g, ""),
				taxCostPrice : (data[i].人进价单价||"").replace(/\s+/g, ""),
				noTaxCostPrice : (data[i].库存商品单价||"").replace(/\s+/g, ""),
				expEnterPrice : (data[i].采购成本单价||"").replace(/\s+/g, ""),
				setCostPrice : (data[i].定价成本单价||"").replace(/\s+/g, ""),
