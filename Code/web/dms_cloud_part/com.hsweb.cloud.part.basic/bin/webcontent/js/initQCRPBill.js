...
function deleteGuest(){
    var records = mainGrid.getSelecteds();
    //if(record.auditSign == 1) return;
    if(!records)
    {
        return;
    }
    mainGrid.removeRows(records);
}
var saveUrl = baseUrl
...
function onExport(){
	...
	if(detail && detail.length > 0){
		//多级表头类型
		//setInitExportData(detail,mainGrid.columns,"期初应收应付");
		//单级表头类型 与上二选一
		setInitExportDataNoMultistage( detail,mainGrid.columns,"期初应收应付");
	}
}