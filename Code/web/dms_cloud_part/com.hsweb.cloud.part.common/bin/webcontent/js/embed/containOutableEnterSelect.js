...
var partGridUrl = baseUrl+"com.hsapi.cloud.part.invoicing.stockcal.queryOutableEnterGridWithPage.biz.ext";
var partGrid = null;
...
function onOk()
{
    ...
    if(!parent.addDetail)
    {
        ...
        //需要判断是否已经添加了此配件
        var checkMsg = parent.checkPartIDExists(nodec.partId);//checkcallback(resultData);
        if(checkMsg) 
        {
            nui.confirm(checkMsg, "友情提示",
                function (action) { 
                    if (action == "ok") {
                        parent.addDetail(nodec);//callback(nodec);
                    }else {
                        return;
                    }
                }
        ...
        {
            //弹出数量，单价和金额的编辑界面
            parent.addDetail(nodec);//callback(resultData);
        }
