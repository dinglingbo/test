...
<!--  				    <td colspan="2" style="text-align: right" id="serviceId"  class="" >No:</td>  -->
				...
				<table id="ybk" width="100%">
				  ...
				  <!-- <tr>
				    <td id="storeId">仓库:</td>
				    <td id="planSendDate">预计发货日期:</td>
				    <td id="planArriveDate">预计到货日期:</td>
				  </tr> -->
				</table>
        	</div> 
        	<hr id="se2"/>
	...
	<script type="text/javascript">
		...
		var MainUrl = baseUrl
				+"com.hsapi.cloud.part.invoicing.guestOrder.queryGuestOrderMainList.biz.ext";
		var DetailUrl = baseUrl
				+"com.hsapi.cloud.part.invoicing.guestOrder.getPjGuestOrderDetailListByMainId.biz.ext";
...
// 			});
       ...
       function getDueAmt(pr,pp){
		...
		function setDetailData(params,detailParms) {
    		...
    		document.getElementById("spstorename").innerHTML = "预销售单";
    		document.getElementById("guestAddr").innerHTML = "地址:"+params.currCompAddress;
	   		...
	   		if(params.id){
		   		$.post(MainUrl+"?params/id="+params.id+"&params/auditSign="+params.auditSign+"&token="+token,{},function(text){
		   			var formParms =text.guestOrderMainList[0];
    		...
    		if(detailParms.mainId){
	    		...
	    		$.post(DetailUrl+"?mainId="+detailParms.mainId+"&token="+token,{},function(text){
					var data= text.guestOrderDetailList;
					var tBody = $("#tbodyId");
					tBody.empty();
						...
						for(var i = 0; i < data.length; i++ ){ 
							...
							tr.append(
								tds.replace("[index]",i+1 ||"")
									...
									.replace("[comPartCode]",data[i].showPartCode ||data[i].comPartCode ||"")
									.replace("[comPartName]",data[i].comPartName ||"")
									.replace("[comPartBrindId]",data[i].comPartBrandId?brandHash[data[i].comPartBrandId].name :"")
									.replace("[comApplyCarModel]",data[i].showCarModel ||data[i].comApplyCarModel ||"")
									.replace("[comSpec]",data[i].showSpec||data[i].spec ||"")
