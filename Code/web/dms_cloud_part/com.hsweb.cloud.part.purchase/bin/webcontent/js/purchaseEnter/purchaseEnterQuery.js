    ...
    getStorehouse(function(data)
    {
        ...
        getDictItems(dictIdList,function(data)
        {
          ...
          //      nui.get("settType").setData(settTypeIdList);
                ...
                orderTypeList.filter(function(v)
                {
                	orderTypeHash[v.id] = v;
                    return true;
                });
                
                nui.get("orderType").setData(orderTypeList);
                ...
                getAllPartBrand(function(data)
        	    {
        	        ...
        	        getResBtnAuth("cloudPchsEnterQuery_price",function(data) {
		            	if(data != null) {
...
function onDrawCell(e)
{
	...
	//            e.cellHtml = partBrandIdHash[e.value].name||"";
        ...
        case "orgCarBrandId":
            ...
            break;
        case "orderType":
            if(orderTypeHash && orderTypeHash[e.value])
            {
                e.cellHtml = orderTypeHash[e.value].name;
            }
            break;
        case "enterDayCount":
            var row = e.record;
...
function onExport(){
	...
	for(var i=0;i<detail.length;i++){
		...
		if(orderTypeHash[detail[i].orderType]){
			detail[i].orderType = orderTypeHash[detail[i].orderType].name;
		}
