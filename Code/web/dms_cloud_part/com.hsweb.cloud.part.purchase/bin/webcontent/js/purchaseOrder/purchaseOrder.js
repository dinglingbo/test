...
var memberUrl = apiPath + sysApi + "/com.hsapi.system.dict.org.queryMember.biz.ext"; //data
var priceGrid=null;
	...
	nui.ajax({
        ...
        success:function(data)
        {
            ...
            if(data.errCode && data.errCode == 'S'){
                nui.get("orderMan").setData(data.data);
                nui.get("elOrderMan").setData(data.data);
            }
...
function onordermanchanged(e) {
	nui.get("deptId").setValue(e.selected.deptId);
...
function onAdvancedSearchOk() {
	...
	// 往来单号
	if (searchData.elCode) {
		searchData.code = nui.get("elCode").getValue();
	}
	// 配件ID
	if (searchData.elPartId) {
		searchData.partId = nui.get("elPartId").getValue();
	}
	// 仓库
...
function save() {
	...
	if(Object.getOwnPropertyNames(storeLimitMap).length >0){
		if(!storeLimitMap.hasOwnProperty(data.storeId) && storeHash[data.storeId]){
			showMsg("没有选择"+storeHash[data.storeId].name+"的权限","W");
			return;
		}
	}
...
//		}
		...
		var newRow = {
			...
			orgCarBrandId: part.orgCarBrandId
		};
...
function onStoreIdValueChange(e){
	...
	var sourceType = basicInfoForm.getData().sourceType;
	// 预售单
	if (sourceType == 5) {
		nui.get("storeId").setValue(e.oldValue);
		showMsg("受理预售单不能修改仓库","W");
		return;
	}
...
function getStoreLimit(){
	storeLimitMap={};
	var orderMan = currUserName;//nui.get('orderMan').value;
	if(!orderMan){
		return;
	}
	nui.ajax({
...
//			var storeId= params.storeId;
			...
			var newRow = {
				...
				orgCarBrandId: row.orgCarBrandId
			};
...
function importPart(){
    ...
    nui.open({
        ...
        {
            var iframe = this.getIFrameEl();
            iframe.contentWindow.initImportData(main.storeId, function(data,msg){
				if(data && data.length > 0){
...
function adjustOrderQty(){
	...
	nui.open({
        ...
        {
            var iframe = this.getIFrameEl();
            iframe.contentWindow.setInitData(row.id, "order");
...
var delUrl = baseUrl+ "com.hsapi.cloud.part.invoicing.crud.delPurchaseOrder.biz.ext";
function delOrder(){
	var row = leftGrid.getSelected();
	if(!row){
		showMsg("请先选择订单!","W");
	...
	var billStatusId =row.billStatusId;
	if(billStatusId ==4 || billStatusId==3){
		showMsg("此单已有入库记录，不能删除","W");
		return;
	}
	var mainId = row.id;
	var auditSign = row.auditSign;
	
	nui.confirm("是否要删除该订单?", "友情提示", function(action) {
		if (action == "ok") {
			nui.ajax({
				url : delUrl,
				type : "post",
