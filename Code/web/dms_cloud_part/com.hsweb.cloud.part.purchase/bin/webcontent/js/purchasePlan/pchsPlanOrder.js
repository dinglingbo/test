...
function setInitExportData(main, detail){
    ...
    var tds = '<td  colspan="1" align="left">[comPartCode]</td>' +
        ...
        "<td  colspan='1' align='left'>[totalStockQty]</td>" +
        "<td  colspan='1' align='left'>[masterStockQty]</td>" +
        "<td  colspan='1' align='left'>[branchStockQty]</td>" +
        "<td  colspan='1' align='left'>[onWayQty]</td>" +
    ...
    for (var i = 0; i < detail.length; i++) {
        ...
        if(row.partId){
            var tr = $("<tr></tr>");
            tr.append(
            		tds.replace("[comPartCode]", detail[i].comPartCode?detail[i].comPartCode:"")
                         .replace("[comFullName]", detail[i].comFullName?detail[i].comFullName:"")
                         
                         .replace("[totalStockQty]", detail[i].totalStockQty?detail[i].totalStockQty:"")
                         .replace("[masterStockQty]", detail[i].masterStockQty?detail[i].masterStockQty:"")
                         .replace("[branchStockQty]", detail[i].branchStockQty?detail[i].branchStockQty:"")
                         .replace("[onWayQty]", detail[i].onWayQty?detail[i].onWayQty:"")
    ...
    method5('tableExcel',"计划采购"+serviceId,'tableExportA');
}
