...
function onLeftGridDrawCell(e)
{
    ...
    switch (e.field){
        ...
        case "billStatusId":
            ...
            if(e.value == 2 || e.value == 3 || e.value == 4 || e.value == 5) {
        ...
        nui.confirm("您正在编辑数据,是否要继续?", "友情提示",
            function (action) { 
                if (action == "ok") {
                    ...
                    var newRow = { serviceId: "新销售订单", auditSign: 0, billStatusId: 0};
...
function addDetail(row,data,ck)
{
    ...
    enterDetail.orgCarBrandId = data.orgCarBrandId;
...
function audit()
{
  ...
  //开启额度管理
	...
	nui.confirm("是否确定提交?", "友情提示", function(action) {
		if (action == "ok") {
		    nui.mask({
		        el: document.body,
		        cls: 'mini-mask-loading',
...
var unAuditUrl = baseUrl+"com.hsapi.cloud.part.invoicing.crud.unAuditPjSellOrderToOut.biz.ext";
function unAudit()
{
