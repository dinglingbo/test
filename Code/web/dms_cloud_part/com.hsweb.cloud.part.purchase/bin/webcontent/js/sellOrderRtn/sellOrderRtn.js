...
$(document).ready(function(v) {
    ...
    var dictDefs ={"rtnReasonId":"DDT20130703000073", "billTypeId":"DDT20130703000008", "settleTypeId":"DDT20130703000035", "orgCarBrandId": "10444", "businessTypeId": "10445"};
...
function onAdvancedSearchOk() {
    ...
    //销售员
    if(searchData.searchOrderMan)
    {
    	searchData.orderMan = nui.get("searchOrderMan").getValue();
    }
	// 仓库
...
function onBusinessTypeIdValueChanged(e) {
	var data = e.selected;
	getBusinessGuest(data.property2);
...
function save() {
	...
	if(businessTypeId!= "1591934420679" && businessTypeId != "1591934420684") {//平台销售，保险平台或是保险业务时，需要指定二级（结算单位），转第三方结算
		if(nui.get("settleGuestId").getValue() == null || nui.get("settleGuestId").getValue() == "") {
			showMsg("请选择销售类型对应的结算单位!","W");
			return;
		}
...
{
    nui.open({
        // targetWindow: window, webPath + contextPath + "/com.hsweb.cloud.part.common.orderBillChoose.flow?token="+token,
        url: webPath + contextPath + "/com.hsweb.cloud.part.common.sellOrderOutSelect.flow?token="+token,
        title: "销售出库记录选择", width: 1200, height: 560,
        allowDrag:true,
        allowResize:true,
        onload: function ()
        {
            ...
            var data = {
                ...
                settleGuestId: nui.get("settleGuestId").getValue(),
                businessTypeId: nui.get("businessTypeId").getValue(),
                storeId: nui.get("storeId").getValue(),
                billTypeId: nui.get("billTypeId").getValue()
            };
...
function addDetail(rows)
{
    ...
    for(var i=0; i<rows.length; i++){
        ...
        var newRow = {
            ...
            comUnit : row.comUnit,
            orderQty : row.outableQty,
            orderPrice : row.orderPrice,
            orderAmt : row.outableQty * row.orderPrice,
            showPrice : row.showPrice,
            showAmt : row.outableQty * row.showPrice,
            storeId : nui.get("storeId").getValue(),
...
var unAuditUrl = baseUrl+"com.hsapi.cloud.part.invoicing.crud.unAuditPjSellOrderRtn.biz.ext";
function unAudit()
{
    var data = basicInfoForm.getData();
    ...
    var mainId = data.id;
    if(billStatusId != 2){
		showMsg("【已提交】状态下的单才可以返单!","W");
        return;
    }

    nui.confirm("是否确定返单?", "友情提示", function(action) {
		if (action == "ok") {
			nui.mask({
		        el: document.body,
		        cls: 'mini-mask-loading',
