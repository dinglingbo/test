...
//	    } 
	...
	var dictDefs = {"orgCarBrandId": "10444"};//{"billTypeId":"DDT20130703000008", "settleTypeId":"DDT20130703000035"};
...
var updatePrintUrl = baseUrl+"com.hsapi.cloud.part.invoicing.paramcrud.updatePrintTimes.biz.ext";
function updatePrint(id, orderTypeId){
	nui.ajax({
		url : updatePrintUrl,
		type : "post",
		async: false,
...
function getSearchParam() {
    ...
    params.serviceId =nui.get("searchServiceId").getValue()||"";
...
function auditToEnter(){
	...
	if(orderTypeId== 1){  //采购单
		var id = data.id||0;
		orderEnter(id);	
	}else if(orderTypeId ==4){ //销售退货单
		var id = data.id||0;
		rtnOrderEnter(id);	
	}
...
function onPrint(){
	...
	var openUrl = webPath + contextPath+"/purchase/stockInTask/stockInTaskPrint.jsp";
...
function setInitExportData(main, detail){
    ...
    method5('tableExcel',"入库单据"+serviceId,'tableExportA');
}
...
function adjustOrderQty(){
   ...
   if(row.orderTypeId != 1) {
	...
	nui.open({
       ...
       {
           var iframe = this.getIFrameEl();
           iframe.contentWindow.setInitData(row.id, "stock");
           loadMainAndDetailInfo(row);
       },
       ondestroy: function (action)
       {
...
function onCost(){
	...
	nui.open({
		// targetWindow: window,,
		url : webPath+contextPath+"/com.hsweb.cloud.part.purchase.pchsExpense.flow?token="+token,
		title : "费用登记",
		width : 930,
		height : 560,
		allowDrag : true,
