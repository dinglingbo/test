...
<!--  				    <td colspan="2" style="text-align: right" id="serviceId"  class="" >No:</td>  -->
				...
				<table id="tbody" width="100%"  border="1" style="border: 1px solid #151515; border-collapse:collapse;">
					<tbody>
                        <tr>
							...
							<td id="receiveStoreShelf" width="100">仓位</td>							
						</tr>
                        <tbody id="tbodyId">
						</tbody>
                    </tbody>
            ...
            <div>
				...
				<table>
				  ...
				  <tr id="border2">
				    <td id="orderMan"  width="25%">业务员:</td>
				    <td id="storeId" width="25%">出库仓:</td>
	...
	<script type="text/javascript">
		...
		var compBrandList = [];
		var compBrandHash = {};
		var dictDefs ={"orgCarBrandIdE":"10444"};//"billTypeIdE":"DDT20130703000008", "settleTypeIdE":"DDT20130703000035", 
		var baseUrl = apiPath + cloudPartApi + "/";
		var supplierUrl=apiPath + cloudPartApi + "/"+"com.hsapi.cloud.part.baseDataCrud.crud.queryGuestList.biz.ext";
		var MainUrl = baseUrl
				+"com.hsapi.cloud.part.invoicing.svr.queryPjShiftOrderMainList.biz.ext";
		var DetailUrl = baseUrl
				+"com.hsapi.cloud.part.invoicing.svr.queryPjShiftOrderDetailList.biz.ext";
    ...
    //		$('#dueAmt').text("客户欠款金额:"+dueAmt+"元");
    		...
    		document.getElementById("spstorename").innerHTML = "移仓入库单";
    		document.getElementById("guestAddr").innerHTML = "地址:"+params.currCompAddress;
	   		...
	   		if(params.id){
		       	...
		       	//	$('#guestFullName').text("客户:"+formParms.guestFullName);
   				    ...
   				    $('#storeName').text(formParms.receiveStoreId?"入库仓:"+storeHash[formParms.receiveStoreId].name :"入库仓:");
   				    $('#storeId').text(formParms.storeId?"出库仓:"+storeHash[formParms.storeId].name :"出库仓:");
		    		...
		    		if(formParms.orderMan != null){
		    			$('#orderMan').text("业务员:"+formParms.orderMan);
		    		}
		       		/* 
		     		if(billTypeIdHash){
    		...
    		if(detailParms.mainId){
	    		...
	    		$.post(DetailUrl+"?params/mainId="+detailParms.mainId+"&token="+token,{},function(text){
					var data= text.detailList;
					var tBody = $("#tbodyId");
					tBody.empty();
					var tds='<td align="center">[index]</td>'+
							'<td align="center">[orgCarBrandId]</td>'+
							...
							'<td align="center" >[receiveStoreShelf]</td>';					
							
						for(var i = 0; i < data.length; i++ ){ 
							var tr=$("<tr></tr>");
							tr.append(
								tds.replace("[index]",i+1 ||"")
									.replace("[orgCarBrandId]",data[i].orgCarBrandId?compBrandHash[data[i].orgCarBrandId].name :"")
									...
									.replace("[receiveStoreShelf]",data[i].receiveStoreShelf ||""));
							tBody.append(tr);
