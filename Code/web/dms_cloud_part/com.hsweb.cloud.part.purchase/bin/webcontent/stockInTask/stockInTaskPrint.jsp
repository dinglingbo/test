...
<!--  				    <td colspan="2" style="text-align: right" id="serviceId"  class="" >No:</td>  -->
				  ...
				  <tr id="border1">
				    ...
				    <td style="font-size:8px;" id="orderType" align="left">订单类型:</td>	    
				  </tr>
				</table>
				<hr id="se"/>
				<table id="ybk" width="100%">
				   <tr>
				    <td colspan="2" width="33.3%" id="guestFullName">供应商:</td>
            ...
            <div>
				...
				<table>
				  ...
				  <tr id="border2">
				    ...
				    <td id="" width="25%">发货人:</td>
				    <td id="" width="25%">审核人:</td>
				    <td id="" >客户确认签字:</td>	
				  </tr>
				  <!-- <tr id="border2">
	...
	<script type="text/javascript">
		...
		var compBrandList = [];
		var compBrandHash = {};
		var dictDefs ={"billTypeIdE":"DDT20130703000008", "settleTypeIdE":"DDT20130703000035", "orgCarBrandIdE":"10444"};
    		...
    		document.getElementById("guestAddr").innerHTML = "地址:"+params.currCompAddress;
	   		...
	   		if(params.id){	
		   		$.post(MainUrl+"?params/id="+params.id+"&params/auditSign="+params.auditSign+"&token="+token,{},function(text){
		     		...
		     		if(billTypeIdHash && billTypeIdHash[formParms.billTypeId]){
		     			$('#billTypeId').text("票据类型:"+billTypeIdHash[formParms.billTypeId].name);
		     		}
		    		if(settleTypeIdHash && settleTypeIdHash[formParms.settleTypeId]){
		    			$('#settleTypeId').text("结算方式:"+settleTypeIdHash[formParms.settleTypeId].name);
		    		}
		    		if(formParms.orderType) {
		    			$('#orderType').text(formParms.orderType == 1 ? "订单类型:常规订单" : formParms.orderType == 2 ? "订单类型:备货订单" : "订单类型:急件订单");
		    		}
		    		if(formParms.remark) {
