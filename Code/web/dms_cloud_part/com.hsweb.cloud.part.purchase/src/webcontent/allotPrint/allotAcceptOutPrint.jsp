...
<!--  				    <td colspan="2" style="text-align: right" id="serviceId"  class="" >No:</td>  -->
	...
	<script type="text/javascript">
		...
		var MainUrl = baseUrl
				+ "com.hsapi.cloud.part.invoicing.allotsettle.queryPjAllotAcceptMainList.biz.ext";
		var DetailUrl = baseUrl
				+ "com.hsapi.cloud.part.invoicing.allotsettle.queryPjAllotAcceptDetailList.biz.ext";
    	...
    	function SetData(params,detailParms){
    		...
    		document.getElementById("spstorename").innerHTML = "调拨出库单";
    		document.getElementById("guestAddr").innerHTML = "地址:"+params.currCompAddress;
	   		...
	   		if(params.id){	
		   		$.post(MainUrl+"?params/id="+params.id+"&params/auditSign="+params.auditSign+"&token="+token,{},function(text){
		   			var formParms =text.pjAllotAcceptMainList[0];
		       		$('#guestFullName').text("调入方:"+formParms.guestFullName);
		       		...
		       		if(storeHash[formParms.storeId]){
		       			$('#storeId').text("调出仓库:"+storeHash[formParms.storeId].name);
    		...
    		if(detailParms.mainId){
	    		...
	    		$.post(DetailUrl+"?params/mainId="+detailParms.mainId+"&params/auditSign="+detailParms.auditSign+"&token="+token,{},function(text){
					var data= text.pjAllotAcceptDetailList;
					var tBody = $("#tbodyId");
					tBody.empty();
					var tds='<td align="center">[index]</td>'+
						...
						$('#sumQty').text("数量合计:"+parseFloat(sumAcceptQty).toFixed(1));
						setTimeout(function(){
					    	$(".print_btn").hide();
				            document.getElementById("query-table").style.overflow="hidden"
				            window.print();
