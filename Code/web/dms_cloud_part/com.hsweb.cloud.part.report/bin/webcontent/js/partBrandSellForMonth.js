...
var rightGridUrl = baseUrl + "com.hsapi.cloud.part.report.report.querySellPartBrandForMonth.biz.ext";
    ...
    rightGrid.on("preload",function(e)
    {
        ...
        for(var i=0;i<brandList.length;i++)
        {
            for(var j=0;j<sellList.length;j++)
            {
                if(brandList[i].partBrandId == sellList[j].partBrandId)
                {
                    for(var k=0; k<keySellList.length; k++){
                        brandList[i][keySellList[k]] = sellList[j][keySellList[k]];
                    }
                }
            }
            ...
            for(var j=0;j<keySellQtyList.length;j++){
                var sellQty = brandList[i][keySellQtyList[j]]||0;  //销售数量
                var enterAmt = brandList[i][keyEnterAmtList[j]]||0;  //销售成本
                var sellAmt = brandList[i][keySellAmtList[j]]||0;  //销售金额

                var rtnQty = brandList[i][keyRtnQtyList[j]]||0;//退货数量
                var rtnEnterAmt = brandList[i][keyRtnEnterAmtList[j]]||0;//退货数量
                var rtnAmt = brandList[i][keyRtnAmtList[j]]||0;//退货金额

                if(sellQty!=0 || rtnQty!=0){
                    brandList[i][keyTrueQtyList[j]] = sellQty - rtnQty;
                }
                if(sellAmt!=0 || rtnAmt!=0){
                    brandList[i][keyTrueAmtList[j]] = sellAmt - rtnAmt;
                }
                if(enterAmt!=0 || rtnEnterAmt!=0){
                    brandList[i][keyTrueEnterAmtList[j]] = enterAmt - rtnEnterAmt;
                }
                if(enterAmt!=0 || rtnEnterAmt!=0 || sellAmt!=0 || rtnAmt!=0){
                    brandList[i][keyTrueProfitList[j]] = sellAmt - enterAmt - rtnAmt + rtnEnterAmt;
                }
            ...
            if(sumSellQty!=0){
                brandList[i]["sumSellQty"] = sumSellQty;
            }
            if(sumSellAmt!=0){
                brandList[i]["sumSellAmt"] = sumSellAmt;
            }
            if(sumRtnQty!=0){
            ...
            if(sumEnterAmt!=0 || sumRtnEnterAmt!=0){
                brandList[i]["sumTrueCost"] = sumTrueCost;
            }
            if(sumEnterAmt!=0 || sumRtnEnterAmt!=0 || sumSellAmt!=0 || sumRtnAmt!=0){
                brandList[i]["sumTrueProfit"] = sumTrueProfit;
            }
