...
//                       5:"调拨申请",6:"调拨受理",7:"调出退回",8:"调入退回",9:"费用"};
var enterTypeIdHash = {1:"采购订单",2:"销售订单",3:"采购退货",4:"销售退货",9:"费用"};

//var balaTypeList=[{id:"1",name:"采购对账"},{id:"2",name:"销售对账"},{id:"3",name:"调入对账"},{id:"4",name:"调出对账"},{id:"5",name:"费用对账"}];
var balaTypeList=[{id:"1",name:"采购对账"},{id:"2",name:"销售对账"},{id:"5",name:"费用对账"}];
...
function loadMainAndDetailInfo(row)
{
    if(row) {    
       ...
       var mainId = row.id;
        ...
        if(row.balaType == 5){
			rightGrid.showColumn('expenseTypeCode');
			rightGrid.showColumn('logisticsNo');
			rightGrid.hideColumn('billTypeId');
		}else {
			rightGrid.hideColumn('expenseTypeCode');
			rightGrid.hideColumn('logisticsNo');
			rightGrid.showColumn('billTypeId');
		}
...
function onAdvancedSearchOk()
{
    ...
    //供应商
    if(searchData.billServiceId)
    {
    	searchData.billServiceId = nui.get("billServiceId").getValue();
    }
    //供应商
    if(searchData.elBalaType)
    {
    	searchData.balaType = nui.get("elBalaType").getValue();
    }
    //订单单号
...
function onShowRowDetail(e) {
    ...
    var expCode = row.expenseTypeCode;
        ...
        nui.confirm("您正在编辑数据,是否要继续?", "友情提示",
            function (action) { 
                if (action == "ok") {
                    ...
                    nui.get("fYear").setValue(nowYear);
                    nui.get("fMonth").setValue(nowMonth+1);
...
function checkBalaType(){
	...
	if(balaType ==5){
		for(var i=0;i<data.length;i++){
			if(data[i].expenseTypeCode == null || data[i].expenseTypeCode == ""){
				showMsg("明细的业务类型与对账类型不匹配","W");
				return false;
			}
		}
...
function addBill(){
    ...
    if(balaType==5){
    	selectExpense(guestId,"",function(rows) {
            
    		addExpenseDetail(rows);

        },function(expenseId, serviceId) {
            var rtn = checkBillExpenseExists(expenseId, serviceId);
            return rtn;
        });
    	return;
    }
...
function selectExpense(guestId,orderTypeIdList,callback,checkcallback)
{
    nui.open({
        // targetWindow: window,
        url: webPath+contextPath+"/com.hsweb.cloud.part.settlement.expenseSelect.flow?token="+token,
        title: "费用单选择", width: 1000, height: 560,
        allowDrag:true,
        allowResize:true,
        onload: function ()
        {
...
function addExpenseDetail(rows)
{
    //var iframe = this.getIFrameEl();
    //var data = iframe.contentWindow.getData();
    for(var i=0; i<rows.length; i++){
        ...
        var newRow = {
            ...
            typeCode:row.billTypeCode,
            rpDc: -1,
            billAmt:row.expenseAmt||0,
            voidAmt:0,
            rpAmt : row.noStateAmt||0,
            noStateAmt :row.noStateAmt||0,
            billTypeId:row.billTypeId,
            settleTypeId:row.settleTypeId,
            orderMan:row.creator,
            billDate:row.auditDate,
            remark:row.remark,
            logisticsNo:row.logisticsNo,
            guestId : row.logisticsGuestId,
            guestName :row.fullName,
            expenseId: row.id,	
...
function checkBillExpenseExists(expenseId, serviceId){
    var row = rightGrid.findRow(function(row){
        if(row.expenseId == expenseId) {
            return true;
        }
        return false;
    });
    ...
    if(row) 
    {
        return "业务单"+serviceId+"在对账列表中存在重复数据!";
    }
...
function onExport(){
	...
	if(main.balaType == 5) {
		setExpenseExportData(main,detail);
		return;
	}
...
function onBalaTypeValueChange(e){
	var data = e.selected;
...
function setInitExportData(main,detail,partHash){
	...
	//单据
	var tds = "<td  colspan='1' align='left'>[orderMan]</td>" +
	...
	//if(row.typeCode==2){
		var tr = $("<tr></tr>");
		var tdh= '<td colspan="1" align="center">业务员</td>'+
		...
		for(var j =0;j<sellList.length;j++){
			...
			for(var i=0;i<detail.length;i++) {
				...
				if(sellHash.mainId ==row.billMainId){
					var trp = $("<tr></tr>");
					trp.append(tds.replace("[orderMan]", row.orderMan?row.orderMan:"")
							      ...
							      .replace("[partCode]", sellHash.showPartCode?sellHash.showPartCode:"")
					              ...
					              .replace("[orgCarBrandId]",compBrandHash[sellHash.orgCarBrandId]&&compBrandHash[sellHash.orgCarBrandId].name?compBrandHash[sellHash.orgCarBrandId].name:"")
					              .replace("[businessTypeId]",businessTypeHash[row.businessTypeId]&&businessTypeHash[row.businessTypeId].name?businessTypeHash[row.businessTypeId].name:"")
				                  ...
				                  .replace("[serviceId]",main.serviceId?main.serviceId:"")
		...
		/*if(row.typeCode==3){
			for(var j =0;j<sellList.length;j++){
				...
				for(var i=0;i<detail.length;i++) {
					...
					if(sellHash.mainId ==row.billMainId){
						var trp = $("<tr></tr>");
						trp.append(tdsp.replace("[orderMan]", row.orderMan?row.orderMan:"")
								  	   .replace("[billDate]", row.billDate?format(row.billDate, 'yyyy-MM-dd HH:mm:ss'):"")
							           .replace("[typeCode]", row.typeCode? enterTypeIdHash[row.typeCode]:"")
							           .replace("[billServiceId]", row.billServiceId?row.billServiceId:"")
							           .replace("[partCode]", sellHash.comPartCode?sellHash.comPartCode:"")
						               ...
						               .replace("[unit]", sellHash.comUnit?sellHash.comUnit:"")
						               .replace("[storeId]",storeHash[sellHash.storeId].name?storeHash[sellHash.storeId].name:"")
							           .replace("[orgCarBrandId]",compBrandHash[sellHash.orgCarBrandId].name?compBrandHash[sellHash.orgCarBrandId].name:"")
							           .replace("[businessTypeId]",businessTypeHash[row.businessTypeId].name?businessTypeHash[row.businessTypeId].name:"")
						               .replace("[qty]",sellHash.orderQty?sellHash.orderQty:"")
				...
				for(var i=0;i<detail.length;i++) {
					...
					if(pchHash.mainId ==row.billMainId){
						...
						trp.append(tds.replace("[orderMan]", row.orderMan?row.orderMan:"")
						               ...
						               .replace("[typeCode]", row.typeCode? enterTypeIdHash[row.typeCode]:"")
						               .replace("[billServiceId]", row.billServiceId?row.billServiceId:"")
						               .replace("[partCode]", pchHash.comPartCode?pchHash.comPartCode:"")
							           ...
							           .replace("[orgCarBrandId]",compBrandHash[pchHash.orgCarBrandId]&&compBrandHash[pchHash.orgCarBrandId].name?compBrandHash[pchHash.orgCarBrandId].name:"")
							           .replace("[businessTypeId]",businessTypeHash[row.businessTypeId]&&businessTypeHash[row.businessTypeId].name?businessTypeHash[row.businessTypeId].name:"")
...
function setExpenseExportData(main,detail){
	...
	var tds = "<td  colspan='1' align='left'>[orderMan]</td>" +
			  ...
			  '<td  colspan="1" align="left">[typeCode]</td>' +
			  "<td  colspan='1' align='left'>[billServiceId]</td>" +
			  '<td  colspan="1" align="left">[expenseTypeCode]</td>' +
			  "<td  colspan='1' align='left'>[amt]</td>" +
			  "<td  colspan='1' align='left'>[logisticsNo]</td>" +
			  "<td  colspan='1' align='left'>[detailRemrk]</td>" +
			  "<td  colspan='1' align='left'>[serviceId]</td>" +
			  "<td  colspan='1' align='left'>[remark]</td>" ;
		...
		var tdh= '<td colspan="1" align="center">业务员</td>'+
				 ...
				 '<td colspan="1" align="center">业务类型</td>'+
				 '<td colspan="1" align="center">业务单号</td>'+
				 '<td colspan="1" align="center">费用项目</td>'+ 
				 '<td colspan="1" align="center">金额</td>'+
				 '<td colspan="1" align="center">物流单号</td>'+
				 '<td colspan="1" align="center">运费备注</td>'+
				 '<td colspan="1" align="center">对账单号</td>'+
				 '<td colspan="1" align="center">备注</td>';
		tr.append(tdh);
		tableExportContent.append(tr);
		for(var i=0;i<detail.length;i++) {
			var row = detail[i];
				...
				trp.append(tds.replace("[orderMan]", row.orderMan?row.orderMan:"")
						      ...
						      .replace("[typeCode]", row.typeCode? enterTypeIdHash[row.typeCode]:"")
						      .replace("[billServiceId]", row.billServiceId?row.billServiceId:"")
						      .replace("[expenseTypeCode]", row.expenseTypeCode?InComeIdHash[row.expenseTypeCode]?InComeIdHash[row.expenseTypeCode].name:"":"")
			                  .replace("[amt]",row.billAmt?row.billAmt:0)
			                  .replace("[logisticsNo]",row.logisticsNo?row.logisticsNo:"")
			                  .replace("[detailRemrk]",row.remark?row.remark:"")
			                  .replace("[serviceId]",main.serviceId?main.serviceId:"")
				              .replace("[remark]",main.remark?main.remark:""));
				tableExportContent.append(trp);
		}
...
function setInitExportDataCopy(main,detail,partHash){
	var pchList = partHash.pchList;
	var sellList = partHash.sellList;
	document.getElementById("eServiceId").innerHTML = main.serviceId?main.serviceId:"";
	document.getElementById("eRemark").innerHTML = main.remark?main.remark:"";
