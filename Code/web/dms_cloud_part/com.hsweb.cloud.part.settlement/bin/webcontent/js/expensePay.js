...
$(document).ready(function(v)
{
    ...
    getInComeExpenses(function(data) {
        ...
        nui.get("elBillTypeId").setData(IClist);
        ...
        ondestroy: function (action)
        {
            if(action == 'ok')
            {
                ...
                if(supplier){
                    ...
                    var newRow = {guestId: guestId, guestName: fullName, rpDc: -1, accountTypeId: 2, settleType:"应付"};
...
function doPrint(){//打印费用报销单
    ...
    if(arr.length){
        if(arr[0].auditSign){
            ...
            nui.open({
                url: webBaseUrl + "com.hsweb.cloud.part.purchase.expensePayPrint.flow",
                title: "打印",
                width: "100%",
                height: "100%",
                onload: function () {
...
function onExport(){
	...
	for(var i=0;i<detail.length;i++){
		if(accountHash[detail[i].balaAccountId]){
			detail[i].balaAccountId = accountHash[detail[i].balaAccountId].name;
		}
		if(accountTypeHash[detail[i].balaTypeCode]){
			detail[i].balaTypeCode = accountTypeHash[detail[i].balaTypeCode].name;
		}
		if(enterTypeIdHash[detail[i].billTypeId]){
			detail[i].billTypeId = enterTypeIdHash[detail[i].billTypeId].name;
		}
		if(auditSignHash[detail[i].auditSign]){
	...
	if(detail && detail.length > 0){
		...
		//单级表头类型 与上二选一
		setInitExportDataNoMultistage( detail,mainGrid.columns,"费用支出单");
	}
}
