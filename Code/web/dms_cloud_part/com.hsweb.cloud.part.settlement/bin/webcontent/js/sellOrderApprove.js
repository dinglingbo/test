...
$(document).ready(function(v)
{
    ...
    var dictDefs ={"billTypeId":"DDT20130703000008", "settleTypeId":"DDT20130703000035", "businessTypeId": "10445"};
...
function updStatement()
{
    ...
    nui.confirm("是否确定修改?", "友情提示", function(action) {
		if (action == "ok") {
...
function onExport(){
	var detail = nui.clone(notStatementGrid.getData());
	//多级
	//exportMultistage(notStatementGrid.columns);
	//单级
	exportNoMultistage(notStatementGrid.columns);
	...
	if(detail && detail.length > 0){
		...
		//单级表头类型 与上二选一
		setInitExportDataNoMultistage( detail,notStatementGrid.columns,"销售订单审核导出"+(format((new Date()), 'yyyy-MM-dd HH:mm:ss')));
	}
