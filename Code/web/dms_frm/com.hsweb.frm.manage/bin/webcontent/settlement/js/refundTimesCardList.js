...
+"com.hsapi.repair.baseData.cardTimes.queryCardTimesListByGuestId.biz.ext";
		...
		success : function(text) {
			...
			card = returnJson.params;
				...
				for(var i = 0;i<returnJson.params.length;i++){
					var tureRefundAmt =0;
					if(returnJson.params[i].userTimes==0){
						 tureRefundAmt = returnJson.params[i].sellAmt;
					} else{
						 tureRefundAmt = 0;
					}
					cardList[i] = {
							value:	returnJson.params[i].id,
							text :returnJson.params[i].cardName+"--可退金额:"+tureRefundAmt
					};
				}
