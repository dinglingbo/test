	        	...
	        	if(text.errCode == "S"){
    				...
    				for(var i = 0 , l = data.length ; i < l ; i++){
    					       ...
    					       if(data[i].billItemId == 0){
    					          if(itemIndex != -1){//第一次进来不添加行
			    				    ...
			    				    tds.replace("[id]", data[itemIndex].orderIndex)
				    				  ...
				    				  .replace("[unitPrice]",unitPriceTatol)
				    				  .replace("[amt]",data[itemIndex].amt)
				    				  .replace("[rate]",rate)
				    				  .replace("[subtotal]",data[itemIndex].subtotal));
				    				tBody.append(tr);
    					          }
    					          itemIndex = i;
    					         ...
    					         if(itemIndex != -1){
					                if(params.type){
					                     if(data[itemIndex].itemId == data[i].billItemId){
    					                    //金额
					                     ...
					                     if(data[itemIndex].id == data[i].billItemId){
    					                    //金额
    					                    data[itemIndex].amt = parseFloat(data[itemIndex].amt) + parseFloat(data[i].amt);
    					                    //小计
    					                    data[itemIndex].subtotal = parseFloat(data[itemIndex].subtotal) + parseFloat(data[i].subtotal);
	                 ...
	                 if(params.type){
    				  ...
    				  .replace("[prdtName]",itemName)
    				  .replace("[qty]",itemTime)
    				  .replace("[unitPrice]",unitPriceTatol2)
    				  .replace("[amt]",data[itemIndex].amt)
    				  .replace("[rate]",rate2)
    				  .replace("[subtotal]",data[itemIndex].subtotal));
    				  tBody.append(tr);
