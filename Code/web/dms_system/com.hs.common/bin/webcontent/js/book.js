$(function() {
	...
	for (var i = 0; i < $('.runPage').length; i++) {
		$('.runPage').eq(i).css('z-index', 21 - 2 * i);
		$('.runPage').eq(i).children('div').css('z-index', 21 - 2 * i);
		$('.runPage').eq(i).children('img').css('z-index', 20 - 2 * i);
	}
	;
	...
	function zIndexLast(index, element) {
		if (index == 0) {
			element.css('z-index', 21 - 2 * index);
		}
		;
		setTimeout(function() {
			element.css('z-index', 21 - 2 * index);
			element.children('div').css('z-index', 21 - 2 * index);
			element.children('img').css('z-index', 20 - 2 * index);
		}, 1000);
	}
});