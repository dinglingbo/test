...
<%@page import="org.gocom.bps.web.bizform.ServiceFactory"%>
...
<%
	...
	if(StringUtils.equals("W", workItem.getUrlType())){//web页面，以/开头，不包含应用名的相对路径
		...
		if(actionURL == null) throw new Exception("**"+I18nUtil.getMessage(request, "bps.bizform.CustomURLIsEmpty")+"**");
		...
		if(flowName == null || flowName.trim().length() == 0) throw new Exception("**"+I18nUtil.getMessage(request, "bps.bizform.CustomURLIsEmpty")+"**");
...
<% 		
		...
		request.getRequestDispatcher("defaultTask.jsp").forward(request, response);
	}
	out.clear();
