...
<table cellpadding="0" cellspacing="0"  bgcolor="#EEF3F6" style="width: 100%;height: 100%;table-layout:fixed;">
	<tr>
		<td class="tree_head"><h2><b:message key="catalog_process_config_tree_jsp.busi_proc_cfg"/></h2></td>
	</tr>
	<tr>
...
<script>
   ...
   function nodeRefresh(node){
	...
	function showCatalogDesc (node) {
		parent.frames["right"].window.location.href = "com.primeton.bps.web.composer.bizflowmgr.bizProcessConfigFrame.flow?_eosFlowAction=desc";
	}
   ...
   function queryProcessDef(node){
		...
		if (param != 1) {
			try{
				...
				parent.frames["right"].window.location.href="com.primeton.bps.web.composer.bizflowmgr.bizProcessCustomMgr.flow?_eosFlowAction=init&page/isCount=true&queryCatalogUUID=" + param + "&treeType=2&permType="+permType;
			}catch(e){
			 alert("query error!");            
			}
	    } else 
	        parent.frames["right"].window.location.href="com.primeton.bps.web.composer.bizflowmgr.bizProcessConfigFrame.flow?_eosFlowAction=desc_noopt";
	...
	function queryProcessDefDetail(node){
		...
		try{
			parent.frames["right"].window.location.href="com.primeton.bps.web.composer.bizflowmgr.bizProcessCustomMgr.flow?_eosFlowAction=expand&wfbizprocess/processDefID=" + param + "&wfbizprocess/processDefName=" + param1 + "&queryCatalogUUID=" + param2 +"&treeType=2";
		}catch(e){
			 alert("query error!");            
		}
	}
