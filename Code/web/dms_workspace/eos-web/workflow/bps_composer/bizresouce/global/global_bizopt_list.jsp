...
<%
// 全选
String selectAll = ResourcesMessageUtil.getI18nResourceMessage("permission_common.select_all");
%>
<html>
<!-- 
...
-->
<head>
<title><b:message key="global_bizopt_list_jsp.global_bzi_op_mgr"/></title><!-- 全局业务操作管理 -->
<script>
	function toShowDesc(scope){
		...
		if(scope!=""){
			for(i=0;i<4;i++){
				...
				if(tmp=="1"){
					scopeDesc+=dictScope(i+1)+'<b:message key="permission_common.semicolon"/>';					
				}
			}
		}
	...
	function dictScope(v){
		if(v==1){
			return '<b:message key="global_bizopt_list_jsp.busi_op_scope_v1"/>';
		}
		if(v==2){
			return '<b:message key="global_bizopt_list_jsp.busi_op_scope_v2"/>';
		}
		if(v==3){
			return '<b:message key="global_bizopt_list_jsp.busi_op_scope_v3"/>';
		}
		if(v==4){
			return '<b:message key="global_bizopt_list_jsp.busi_op_scope_v4"/>';
		}
	}
</script>
</head>
<body>
  <h:form name="viewlist1" action="com.primeton.bps.web.composer.bizresouce.GlobalBizOptMgr.flow" method="post">
...
<table border="0" cellpadding="0" cellspacing="0" class="workarea" width="100%">
  <tr>
  	<td class="workarea_title" valign="middle"><h3><b:message key="global_bizopt_list_jsp.busi_op_list"/></h3></td>
  </tr>
  <tr> 
   	<td width="100%" >
   	<table class="EOS_TABLE" width="100%" >
        ...
        <tr>
          <th width="5%" style="white-space:nowrap">
            <div style="word-break:keep-all;"><h:checkbox name="chkall" onclick="list_selectAll()" title="<%=selectAll%>" /><%=selectAll%></div>
          </th>
          <th width="5%" style="white-space:nowrap">
        ...
        <tr>
          ...
          <div id="listleft" style="white-space:nowrap">
            ...
            <l:present property="buttons">
            	...
            	<input id="openExtendButtonDialog" type="button" value="<b:write iterateId='buttons' property='label'/>" 
            ...
            <div id="listright" style="white-space:nowrap">
              <l:equal property="page/isCount" targetValue="true">
                <!-- 共 --><b:message key="permission_common.page_info1"/>
                <b:write property="page/count"/>
                <!-- 条记录 第 --><b:message key="permission_common.page_info2"/>
                <b:write property="page/currentPage"/>
                <!-- 页/ --><b:message key="permission_common.page_info3"/>
                <b:write property="page/totalPage"/>
                <!-- 页 --><b:message key="permission_common.page_info4"/>
              </l:equal>
              <l:equal property="page/isCount" targetValue="false">
                <!-- 第 --><b:message key="permission_common.page_info5"/>
                <b:write property="page/currentPage"/>
                <!-- 页 --><b:message key="permission_common.page_info6"/>
              </l:equal>
  ...
  <script>
    ...
    function showReference() {
      ...
      var strUrl = "com.primeton.bps.web.composer.bizresouce.GlobalBizOptMgr.flow?_eosFlowAction=showReference&resUUID="+params[0];
    ...
    function putinRecord() {
      ...
      showModalCenter(encodeURI(strUrl),"",refreshList,500,350,'<b:message key="permission_common.btn_submit"/>');//提交
    }
    ...
    function pickupRecord() {
      ...
      showModalCenter(encodeURI(strUrl),"",refreshList,500,350,'<b:message key="permission_common.btn_extract"/>');//提取
    }
