...
<table border="0" cellpadding="1" cellspacing="1" class="workarea" width="100%">
  <tr> 
    <td class="workarea_title" valign="middle"><h3><b:message key="add_process_server_jsp.title"/></h3></td>
  </tr>
  <tr> 
    <td width="100%">
    ...
    <tr>
		<td class="form_label" width="20%"><b:message key="add_process_server_jsp.engineID"/></td>
		<td>
			<h:text id="psId" validateAttr="allowNull=false;maxLength=40;type=bpscommname" property="ps/id" onblur="checkInputId(this)"/>
		</td>
	</tr>
	<tr>
		<td class="form_label" width="20%"><b:message key="add_process_server_jsp.engineName"/></td>
		<td>
			<h:text id="psName" validateAttr="allowNull=false;maxLength=40;type=bpscommname" property="ps/name" onblur="checkInputName(this)"/>
		</td>
	</tr>
	<tr>
		<td class="form_label" width="20%"><b:message key="add_process_server_jsp.logicName"/></td>
		<td>
	...
	<tr>
		<td class="form_label"><b:message key="add_process_server_jsp.userName"/></td>
		<td>
			<h:text id="psUserId" validateAttr="allowNull=true;maxLength=20;" property="ps/uddiUserID" onblur="checkInput(this)"/>
		</td>
	</tr>
	<tr>
		<td class="form_label"><b:message key="add_process_server_jsp.pwd"/></td>
		<td>
			<h:password id="psPassword" validateAttr="allowNull=true;maxLength=200;" property="ps/uddiPassword" onblur="checkInput(this)" filter="true" />
		</td>
	</tr>
	<tr>
		<td class="form_label"><b:message key="add_process_server_jsp.desc"/></td>
		<td>
	...
	<script type="text/javascript">		
		$id("_eosFlowAction").value = "addProcessServer";
	</script>
	
	<tr>
		<td colspan="2">
			<input id="btnOK" name="btnOK" class="button" type="button" value="<b:message key="add_process_server_jsp.btnOK"/>" onclick="_confirm()">
			<input id="btnCancel" name="btnCancel" class="button" type="button" value="<b:message key="add_process_server_jsp.btnCancel"/>" onClick="window.location='com.primeton.bps.workspace.frame.processserver.ProcessServerManage.flow'">
			<input id="btnTest" name="btnTest" class="button" type="button" value="<b:message key="add_process_server_jsp.btnTestConn"/>" onclick="_validate()">
		</td>
	</tr>
...
<script type="text/javascript">
    function checkInputId(obj){
		...
		for(var i=0;i<serverIds.length;i++){
			if(obj.value.toLowerCase() == serverIds[i].toLowerCase()){
				f_alert(obj, "<b:message key="add_process_server_jsp.engineIDExist"/>");
				return false;
			}
		}
    ...
    function checkInputName(obj){
		...
		for(var i=0;i<servers.length;i++){
			if(obj.value.toLowerCase() == servers[i].toLowerCase()){
				f_alert(obj, "<b:message key="add_process_server_jsp.engineNameExist"/>");
				return false;
			}
		}
    ...
    function _confirm(){
        ...
        if($id("_eosFlowAction").value=="addProcessServer"){
		    $id("_eosFlowAction").value="doAddProcessServer";
		    $id("addForm").submit();
		}
    }
